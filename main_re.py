...
import logging
from lib.util import load_data_for_re, load_tokenizer, train_val_split_doc, make_idx, make_train_vecs_pipeline, make_test_vecs_pipeline, create_re_labels, save_csv_pipeline, save_re_result
from lib.loop import train_val_loop_re, test_loop_re
import argparse
import json
from lib.util import eval_re_strict


def main():
    logger.info("----------{0}-REの実験を開始----------".format(args.bert_type))
    # 実験データ
    df = load_data_for_re(args)
    # 交差検証用の分割ファイル
    ...
    for fold in range(0, 5, 1):
        ...
        # 検証
        val_vecs, ner_val_labels = make_test_vecs_pipeline(X_val, bert_tokenizer, tag2idx, "RE")
        # テスト
        test_vecs, ner_test_labels = make_test_vecs_pipeline(X_test, bert_tokenizer, tag2idx, "RE")
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler('./logs/Pipeline_RE_{0}.log'.format(args.bert_type), "w")
    logger.addHandler(fh)
    formatter = logging.Formatter('%(asctime)s:%(lineno)d:%(levelname)s:%(message)s')
    fh.setFormatter(formatter)
    sh.setFormatter(formatter)
    # フォルダ作成
    for SAMPLE_DIR in ["./models/{0}/{1}/RE".format(args.task, args.bert_type), "./results/{0}/{1}/RE".format(args.task, args.bert_type)]:
